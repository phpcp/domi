...
layui.define(["jquery", "miniMenu", "element","miniTab", "miniTheme"], function (exports) {
    ...
    // var localStorageName = 'layuimini';
    var miniAdmin = {
        ...
        render: function (options) {
            ...
            localStorageName = options.localStorageName || 'layuimini';
        ...
        listen: function () {
             ...
             * 设置
             */
            $('body').on('click', '[data-cogs]', function () {
                var set_up = layui.data(localStorageName)['set_up'];

                var cache = set_up == undefined?"false":set_up.cache;
                var changeRefresh = set_up == undefined?"false":set_up.changeRefresh;
                var html = '';
                html += '<style>';
                    html += '.functionSrtting_box {padding-top: 15px;}';
                    html += '.functionSrtting_box .layui-word-aux {position: absolute;left: 60px;top: 9px;font-size: 12px;}';
                    html += '.skinBtn {text-align: center;}';
                html += '</style>';
                html += '<div class="functionSrtting_box">';
                    html += '<form class="layui-form">';
                        html += '<div class="layui-form-item">';
                            html += '<label class="layui-form-label">开启Tab缓存</label>';
                            html += '<div class="layui-input-block">';
                                html += '<input type="checkbox" name="cache" lay-skin="switch" lay-text="开|关">';
                                html += '<div class="layui-word-aux">开启后刷新页面不关闭打开的Tab页</div>';
                            html += '</div>';
                        html += '</div>';
                        html += '<div class="layui-form-item">';
                            html += '<label class="layui-form-label">Tab切换刷新</label>';
                            html += '<div class="layui-input-block">';
                                html += '<input type="checkbox" name="changeRefresh" lay-skin="switch" lay-text="开|关">';
                                html += '<div class="layui-word-aux">开启后切换窗口刷新当前页面</div>';
                            html += '</div>';
                        html += '</div>';
                        html += '<div class="layui-form-item skinBtn">';
                            html += '<a href="javascript:;" class="layui-btn layui-btn-sm layui-btn-normal" lay-submit="" lay-filter="settingSuccess">设定完成</a>';
                            html += '<a href="javascript:;" class="layui-btn layui-btn-sm layui-btn-primary" lay-submit="" lay-filter="noSetting">朕再想想</a>';
                        html += '</div>';
                    html += '</form>';
                html += '</div>';
                layer.open({
                    ...
                    success : function(index, layero){
                        $(".functionSrtting_box input[name=cache]").prop("checked",cache=="true" ? true : false);
                        $(".functionSrtting_box input[name=changeRefresh]").prop("checked",changeRefresh=="true" ? true : false);

                        //设定
                        form.on("submit(settingSuccess)",function(data){
                            layui.data(localStorageName, {
                                key: 'set_up'
                                ,value: {
                                    cache:data.field.cache =="on" ? "true" : "false"
                                    ,changeRefresh:data.field.changeRefresh =="on" ? "true" : "false"
                                }
                            });
                            location.reload();
                            return false;
            ...
            $('body').on('click', '[data-clear]', function () {
                ...
                layui.data(localStorageName, null); 
