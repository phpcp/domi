...
layui.define(['jquery', 'layer'], function(exports) {
	...
	TMap.open = function(param) {
		...
		dialogOption.area || (dialogOption.area = ['750px', '560px']);
		dialogOption.shadeClose || (dialogOption.shadeClose = true);
		...
		dialogOption.success = function(layero, tIndex) {
			$(layero).children(".layui-layer-content").load('/static/public/lay-module/TMap/TMap.html', function() {
				init(tIndex);
		...
		function init(myIndex) {
			...
			if( longitude != undefined ){
				document.getElementById("poiCur").value = longitude[0] + "," + longitude[1];
				url12 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?location=" + longitude[0] + "," + longitude[1] + urlSuffix);
				$.getJSON(url12, function(result) {
					
					if (result.result.address != '') {
						document.getElementById("addrCur").value = result.result.address;
						nowCity.children[0].innerHTML = result.result.ad_info.city;
						fixedPoint(longitude[0], longitude[1]);
						map.setZoom(18);
					} else {
						var cityservice = new qq.maps.CityService({
							complete: function(result) {
								nowCity.children[0].innerHTML = result.detail.name;
								fixedPoint(result.detail.latLng.lat,result.detail.latLng.lng);
							}
						});
			...
			qq.maps.event.addListener(map, "click", function(e) {
				...
				$.getJSON(url3, function(result) {
					if (result.result != undefined) {
						...
						fixedPoint(e.latLng.getLat().toFixed(6), e.latLng.getLng().toFixed(6));
					} else {
			...
			var setFlagClicked = function(arr, index) {
					$.each(markerArray, function(n, ele) {
						if (n == index) {
							...
							fixedPoint(latLng.getLat().toFixed(6),latLng.getLng().toFixed(6));
			...
			//定位
			var fixedPoint = function(Lat="",Lng="")
			{
				if( marInfo != false ){
					marInfo.setMap(null);
				}
			 ...
			 //    var anchor = new qq.maps.Point(12, 12),
				...
				// origin = new qq.maps.Point(0, 0),
				// icon = new qq.maps.MarkerImage(marker_n, size, origin, anchor);
			 //    marInfo = new qq.maps.Marker({
			 //        icon: icon,
			 //        map: map,
			 //        position:map.getCenter()
			...
			city.onclick = function(e) {
				...
				if (target.className == "city_name") {
					...
					url2 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?region=" + nowCity.children[0].innerHTML + "&address=" + nowCity.children[0].innerHTML + urlSuffix);
			...
			$("#initPoint").click(function() {
				...
				if( point != ''){
					...
					url13 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?location=" + arr[0] + "," + arr[1] + urlSuffix);
					$.getJSON(url13, function(result) {
						onChoose && onChoose(point, adress, myIndex,result.result);
					});

				}else{
					onChoose && onChoose(point, adress, myIndex,false);
				}
			});
		}
