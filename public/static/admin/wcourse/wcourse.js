layui.use(['form','table','unit_call'], function () {
    ...
    unit_call.delPower('.Screen','wcourse.list');
    ...
    var tableIns = table.render({
        ...
        cols : [[
            ...
            {field: 'type', title: '类型',width:100,sort:true,  align:'center',templet:function(d){
                var color = d.type == 1?'layui-bg-orange':'layui-bg-cyan';
                var text = d.type == 1?'直播技巧':'带货技巧';
                return '<div><span class="layui-badge '+color+'" >'+text+'</span></div>';
            }},
            {field: 'w_name', title: '课程标题',width:100,align:'center',templet:function(d){
                return  '<div><a class="layui-btn layui-btn-xs layui-bg-blue" lay-event="CourseTitle">'+d.w_name+'</a></div>';
            }},
            {field: 'course_img', title: '课程主图',  align:'center',width:100,templet:function(d){
                return '<div id="layer-photos-demo-'+d.id+'"><img style="width:40px;cursor:pointer"  src="/upload/'+d.course_img+'" alt=""/></div>';
            }},
    ...
    table.on('tool(List)', function(obj){
        ...
        if(layEvent === 'CourseTitle'){
            ...
            AddTo(data);
        }else if(layEvent === 'wcourseLowList'){
            var titleName = '《'+data.w_name+'》 -- 课程列表';
            var index = layui.layer.open({
                title : titleName,
                type : 2,
                maxmin:true,
