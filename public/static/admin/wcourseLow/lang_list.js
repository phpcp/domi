layui.use(['form','table','unit_call'], function () {
    ...
    var tableIns = table.render({
        elem: '#List',
        url : 'wcourse-low-lang-ajax?cl_id='+cl_id,
        cellMinWidth : 100,
        height : "full-150",
        page : false,
        ...
        cols : [[
            ...
            {field: 'course_low_video', title: '课程视频',width:150,align:'center',templet:function(d){
                return  '<div><a class="layui-btn layui-btn-xs layui-bg-cyan" lay-event="showVideo">查看视频</a></div>';
            }},
    ...
    table.on('tool(List)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        if(layEvent === 'showVideo'){
            var html = '';
            html += '<div>';
                html += '<video width="100%" height="100%"  controls="controls" autobuffer="autobuffer"  autoplay="autoplay" loop="loop">';
                    html += '<source src="/upload/'+data.course_low_video+'" type="video/mp4"></source>';
                html += '</video>';
            html += '</div>';
            layer.open({
                type:1,
